 ...
 * JWT Service
 * -----------
 * Provides utility functions for working with JSON Web Tokens (JWT) in user authentication flows.
 *
 * Features:
 ...
 *
 * @param payload - The user data to embed in the token (id_user, username, role_id)
 * @returns JWT token as string
 *
 * @example
 * const token = generateToken({ id_user: 1, username: 'user123', role_id: 2 });
...
export const generateToken = (payload: Omit<JWTPayload, 'iat' | 'exp'>): string => {
  try {
    const token = jwt.sign(payload, envConfig.JWT_SECRET || '', {
      ...
      issuer: 'order_management-api',
      audience: 'order_management-client'
    } as jwt.SignOptions);
    return token;
  } catch (error) {
 ...
 * if (payload) {
 *   console.log('User ID:', payload.id_user);
 * }
 */
